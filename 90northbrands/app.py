...
if st.session_state['login_check']==1 :
    tab_imp,tab_sync,tab_trial,tab_so,tab_pnl,tab_sr,tab_sa,tab_exp=st.tabs(["Import_data","Sync_data","trial","Sales Overview","P&L","Style_Review","Suggested Actions","Export_data"])
else :
...
try :
    ...
    with tab_so:
        ...
        db_data_final=db_data.copy()
        db_sales_data_final=db_sales_data.copy()
        ...
        for i in range(tab_len):
            with tab[i]:
                if i==0 :
                    db_data=db_data_final
                    db_sales_data=db_sales_data_final
                else :
                    db_data=db_data_final[db_data_final['channel']==total_channel[i]]
                ...
                #     db_sales_data=db_sales_data_final
                # else :
                #     db_data=db_data_final[db_data_final['channel']==total_channel[tab_check1]]
                #     db_sales_data=db_sales_data_final[db_sales_data_final['channel_x']==total_channel[tab_check1]]
                ...
                total_orders=len(db_sales_data)
                cancelled_orders=len(db_sales_data[db_sales_data['order_status']=='F'])
                shipped_orders=total_orders-cancelled_orders
                RTO=len(db_sales_data[db_sales_data['order_status']=='RTO'])
                in_transit=len(db_sales_data[db_sales_data['order_status'].isin({'SH','L','PK','WP'})])
                delivered=shipped_orders-RTO-in_transit
                total_returns=len(db_sales_data[db_sales_data['returns']==1])
                successful_order=delivered-total_returns
                db_sales_value=db_sales_data[db_sales_data['returns']==0]
                ...
                with st.container(border=True):
                    ...
                    with tab1 :
                        ...
                        db_sales_category=db_sales_data[db_sales_data['order_status']!='F']
                    ...
                    with tab2 :
                        ...
                        db_sales_brand=db_sales_data[db_sales_data['order_status']!='F']
                    ...
                    with tab4 :
                        db_sales_latlong=db_sales_data[db_sales_data['order_status']!='F']
    ...
    with tab_pnl:
        ...
        for i in range(tab_len):
            with tab[i]:
                ...
                total_orders=len(db_sales_data)
                ordered_gmv=sum(db_sales_data['final_amount'])
                cancelled_orders=len(db_sales_data[db_sales_data['order_status']=='F'])
                cancelled_value=db_sales_data.loc[db_sales_data['order_status']=='F','final_amount'].sum()

                shipped_orders=total_orders-cancelled_orders
                shipped_value=ordered_gmv-cancelled_value
                RTO=len(db_sales_data[db_sales_data['order_status']=='RTO'])
                rto_value=db_sales_data.loc[db_sales_data['order_status']=='RTO','final_amount'].sum()
                in_transit=len(db_sales_data[db_sales_data['order_status'].isin({'SH','L','PK','WP'})])
                in_transit_value=db_sales_data.loc[db_sales_data['order_status'].isin({'SH','L','PK','WP'}),'final_amount'].sum()
                delivered=shipped_orders-RTO-in_transit
                delivered_value=shipped_value-in_transit_value-rto_value
                total_returns=len(db_sales_data[db_sales_data['returns']==1])
                returns_value=db_sales_data.loc[db_sales_data['returns']==1,'final_amount'].sum()
                successful_order=delivered-total_returns
                db_sales_value=db_sales_data[db_sales_data['returns']==0]
                db_sales_value=db_sales_value[db_sales_value['order_status']=='C']
                total_sales_value=sum(db_sales_value['final_amount'])
                
                settled_orders_value=db_data.loc[db_data['order_type']=='Forward','customer_paid_amt'].sum()
                settled_order_qty=db_data.loc[db_data['order_type']=='Forward','customer_paid_amt'].count()
                non_settled_orders_value=round(delivered_value-settled_orders_value,2)
                settled_orders_value_net_sales=round(db_data.loc[db_data['returns']==0,'customer_paid_amt'].sum(),2)
                net_sales_order_qty=round(db_data.loc[db_data['returns']==0,'customer_paid_amt'].count(),2)
                ...
                non_settled_orders_count=len(db_sales_data[db_sales_data['order_status'].isin({'C'})])-db_data.loc[db_data['order_type']=='Forward','customer_paid_amt'].count()
                

            
                commission_net_sales=db_data.loc[db_data['returns']==0,'platform_fees'].sum()
                commission_returns=0
                taxes_net_sales=db_data.loc[db_data['returns']==0,'tcs_amount'].sum()+db_data.loc[db_data['returns']==0,'tds_amount'].sum()
                taxes_returns=0
                logistics_net_sales=db_data.loc[db_data['returns']==0,'shipping_fee'].sum()+db_data.loc[db_data['returns']==0,'total_tax_on_logistics'].sum()
                logistics_returns=db_data.loc[db_data['returns']==1,'shipping_fee'].sum()+db_data.loc[db_data['returns']==1,'total_tax_on_logistics'].sum()
                pick_pack_net_sales=db_data.loc[db_data['returns']==0,'pick_and_pack_fee'].sum()
                pick_pack_return=db_data.loc[db_data['returns']==1,'pick_and_pack_fee'].sum()
                payment_gateway_net_sales=db_data.loc[db_data['returns']==0,'payment_gateway_fee'].sum()
                payment_gateway_return=db_data.loc[db_data['returns']==1,'payment_gateway_fee'].sum()
                fixed_fee_net_sales=db_data.loc[db_data['returns']==0,'fixed_fee'].sum()
                fixed_fee_return=db_data.loc[db_data['returns']==1,'fixed_fee'].sum()
                ...
                cogs_net_sales=db_data.loc[db_data['returns']==0,'cost'].sum()
                cogs_return=0
                ...
                with st.container(border=True):
                    db_data_monthly=db_data[['order_created_date','order_type','customer_paid_amt','total_actual_settlement','platform_fees','tcs_amount','tds_amount','shipping_fee','pick_and_pack_fee','fixed_fee','payment_gateway_fee','total_tax_on_logistics','returns','cost']]
    ...
    with tab_sr:
        ...
        try:
            ...
            db_action_items_manual=conn.query("select * from action_items_manual")
        except:
         db_action_items_suggestion=conn.query("select * from action_items_suggestion" )
        ...
        db_search_style_code=db_sales_data['vendor_style_code'].drop_duplicates()
        ...
        if len(search_style_code)>0 :
            db_style_code=db_data[(db_data['vendor_style_code'].isin(search_style_code))]
        
            st.session_state['page_number'] = 1
        else :
            db_style_code=db_data.copy()
        ...
        db_style_code_funnel=db_style_code.copy()
